...
void *BF_Init(byte *key, unsigned keylen)
{
	...
	int i;
        ...
        BLOWFISH_CBC *ctx = (BLOWFISH_CBC*)malloc(sizeof(BLOWFISH_CBC));
...
void BF_Enc(void *handle, byte *chipher, byte *plain, int len)
{
	...
	BLOWFISH_CBC *ctx = (BLOWFISH_CBC*)handle;

	for (j = 0; j < 8; j++) {
		if (j < len) {
			buf[j] = ctx->buf[j] ^ plain[j];
		} else {
			buf[j] = ctx->buf[j];
		}
	}
	encrypt_block(&(ctx->ctx), chipher, buf);
	for (j = 0; j < 8; j++) {
		ctx->buf[j] = chipher[j];
	}
}
...
void BF_Dec(void *handle, byte *plain, byte *chipher)
{
	...
	decrypt_block(&(ctx->ctx), buf, chipher);
	for (j = 0; j < 8; j++) {
		plain[j] = buf[j] ^ ctx->buf[j];
	}
	for (j = 0; j < 8; j++) {
		ctx->buf[j] = chipher[j];
	}
...
void BlowFishTest()
{
	...
	char *plainData = "Good News for WindowsCE Users!";
	BLOWFISH_CBC ctx;
	...
	for (i = 0; i < 1024; i++) {chipher[i] = plain[i] = 0; }
